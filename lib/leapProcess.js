 ...
 * @param {string} [raw.jobSchedules]    - optional: "Leap Job Schedules Report" CSV (preferred crew source)
 * @param {string} [raw.workOrder]       - optional: "Leap Work Order Report" CSV (fallback crew source, often empty)
 */
export function buildBundle(raw, opts = {}) {
  ...
  // ---- crew (from Job Schedules Report — filtered to jobs active on the run date;
  //      falls back to Work Order Report if schedules aren't provided) ----
  const crew = [];
  if (raw.jobSchedules) {
    ...
    js.forEach(s => {
      ...
      // "active today" = the schedule window spans the run date
